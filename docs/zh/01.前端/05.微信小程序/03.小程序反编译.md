...
1. 在电脑端安装模拟器工具，这里以[夜神模拟器](https://www.yeshen.com/)为例。
   ...
   然后在模拟器中安装微信（使用微信打开小程序以下载小程序包）和文件管理器（查看小程序压缩包，这里使用模拟器自带的Amaze）
...
2. 打开Amaze文件管理器，进入到以下路径查找源码包(可以根据下载时间区分出你想要的源码包)，不同的小程序中间的那段hash值是不一样的
...
3. 拷贝小程序包至模拟器共享文件夹，为了方便新建了一个文件夹`wxcode`，将复制的小程序包放到此文件夹下的`东方财富`文件夹
...
2. 在`wxappUnpacker`项目根目录下，执行解包命令
...
解决：有些小程序可能采取了分包策略，所以就有一个主包和多个子包之分。经过多次测试发现，一般主包的文件名的后面的数字比较小，可以尝试以此来区分主包和子包。
